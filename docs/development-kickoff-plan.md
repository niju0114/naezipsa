...
- **정렬 순서 저장 결정:** 사용자가 드래그로 정한 후보 순서를 DB 컬럼으로 저장하기로 결정했다. 기존 팀 규칙("표시 순서는 백엔드에서 관리하지 않음")을 바꾸는 결정이다. 구현 시점·Phase 배치는 미정이며, 위 마이그레이션 불일치 해소가 선행 조건이다.
- **온보딩 미표시 원인:** 수동 확인에 쓴 Google 계정은 이미 `service_purposes=['move','buy']`가 저장돼 있어 모달이 뜨지 않는 것이 정상이다. 새 이메일 계정으로 재확인한다.
